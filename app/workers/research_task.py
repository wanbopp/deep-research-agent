...
class ResearchTaskWorker:
    ...
    async def _execute_claimed(self, task: ResearchTask) -> ResearchWorkerResult:
        ...
        try:
            ...
            await self._finalize_cancelled(task.id)
            ...
            await self._finalize_failed(task.id, error_type=type(error).__name__)
        ...
        if report_data is None or final_state["status"] != ResearchStatus.COMPLETED.value:
            await self._finalize_failed(task.id, error_type="IncompleteResearchState")
            return ResearchWorkerResult.FAILED
        report = ResearchReport.model_validate(report_data)

        async with self._session_factory() as session, session.begin():
            repository = ResearchTaskRepository(session)
            current = await repository.get_worker_view(task.id)
            if current is None:
                raise RuntimeError("claimed research task disappeared")
    ...
    async def _run_graph_until_done_or_cancelled(self, task: ResearchTask) -> ResearchState:
        """竞速执行图与持久取消轮询；取消胜出时中止当前模型协程."""
        graph_task = asyncio.create_task(self._consume_graph(task))
        cancel_task = asyncio.create_task(self._wait_for_cancellation(task.id))
    ...
    async def _consume_graph(self, task: ResearchTask) -> ResearchState:
            ...
            async for update in self._graph.astream(
                ...
                if isinstance(update, Mapping):
                    for node_name, node_update in update.items():
                        await self._record_node_event(task, str(node_name), node_update)

        snapshot = await self._graph.aget_state(runnable_config)
        return cast(ResearchState, snapshot.values)

    async def _record_node_event(self, task: ResearchTask, node_name: str, update: object) -> None:
        ...
        async with self._session_factory() as session, session.begin():
            ...
            current = await repository.get_worker_view(task.id)
            if current is None:
                raise RuntimeError("research task disappeared while recording progress")
            await repository.heartbeat(current, worker_id=self._worker_id)
            await repository.append_event(
                task_id=task.id,
                user_id=task.user_id,
    ...
    async def _wait_for_cancellation(self, task_id: UUID) -> bool:
        ...
        while True:
            ...
            async with self._session_factory() as session, session.begin():
                task = await ResearchTaskRepository(session).get_worker_view(task_id)
                if task is None:
                    return True
                ...
                if loop.time() >= next_heartbeat_at:
                    ...
                    await ResearchTaskRepository(session).heartbeat(task, worker_id=self._worker_id)
                    next_heartbeat_at = loop.time() + self._heartbeat_interval_seconds

    async def _finalize_cancelled(self, task_id: UUID) -> None:
        """把取消中的任务推进到 cancelled 并写最终事件."""
        async with self._session_factory() as session, session.begin():
            repository = ResearchTaskRepository(session)
            task = await repository.get_worker_view(task_id)
            if task is None:
                return
            await repository.finalize(task, status=ResearchTaskStatus.CANCELLED, error_code="CANCELLED_BY_USER")
            await repository.append_event(
                task_id=task.id,
                user_id=task.user_id,
    ...
    async def _finalize_failed(self, task_id: UUID, *, error_type: str) -> None:
        ...
        async with self._session_factory() as session, session.begin():
            ...
            if task is None:
                return
            await repository.finalize(task, status=ResearchTaskStatus.FAILED, error_code=error_type[:128])
            await repository.append_event(
                task_id=task.id,
                user_id=task.user_id,
...
__all__ = ["ResearchCancellationRequested", "ResearchTaskWorker", "ResearchWorkerResult"]
