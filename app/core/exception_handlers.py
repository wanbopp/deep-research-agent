...
def get_request_id() -> str | None:
    ...
    CorrelationMiddleware 在请求进入时设置这个 ContextVar
    handler 不需要自己生成另外一个id，否则响应和日志将无法关联
    """
...
def build_error_content(
    ...
    """构造所有异常处理器共享的错误 body
    detail 为None时，不应该输出"details":"null"
    只有真正存在补充信息时才加入该字段
    """
...
async def validation_exception_handler(
    ...
    """把 FastAPI/Pydantic 校验错误转换为稳定、可读的字段列表。"""
