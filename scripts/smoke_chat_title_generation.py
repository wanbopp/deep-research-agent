...
async def _seed_sessions(
    ...
    async with session_factory() as session:
        async with session.begin():
            ...
            session.add_all((concurrent_session, manual_session, stale_session))
            await session.flush()

    return user.id, concurrent_session.id, manual_session.id, stale_session.id
...
async def _exercise_title_generation(database: str) -> dict[str, object]:
    ...
    try:
        user_id, concurrent_id, manual_id, stale_id = await _seed_sessions(session_factory)
...
def main() -> int:
    ...
    test_database = f"deep_research_chat_title_{uuid4().hex[:10]}"
